...
# TODO Rename these secrets, they are DB specific, not connect specific.. connect just uses them
	...
	@printf "\n"
	@$(call print-prompt)
	kubectl create secret generic cc.ccloud-secrets --namespace=default --from-env-file=$(CCLOUD_SECRET_FILE) --dry-run=client -o yaml > secrets/local-toseal/$*/default-ccloud-secrets.yaml && echo "ready to seal: secrets/local-toseal/$*/default-ccloud-secrets.yaml"
