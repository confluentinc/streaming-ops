...
function ccloud::acl::apply_list() {
    ...
    local controls=$(echo $ACL | jq -r -c '.controls' | @sh)
...
function ccloud::acl::apply_consumer_group() {
  ...
  local result=$(ccloud kafka acl create $permission_flag $service_account_flag $operation_flag $consumer_group_flag --cluster $kafka_id 2>&1) && {
...
function ccloud::acl::apply_cluster_scope() {
  ...
  local result=$(ccloud kafka acl create $permission_flag $service_account_flag $operation_flag --cluster-scope --cluster $kafka_id 2>&1) && {
